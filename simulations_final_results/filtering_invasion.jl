...
for s in 1:5
    global n_load = FileIO.load("consumers_simulation1k_d$D.jld2","s")[s];
    global r_load = FileIO.load("resources_simulation1k_d$D.jld2","r")[s];
    global R_load = FileIO.load("consumption_matrix_simulation1k.jld2","R_save")[s];
    global N = size(n_load[end],2);
    global M = size(r_load[end],2);
    push!(indices,invasion(n_load,r_load,R_load,D,tspanf,Ymag,N,M))
end

FileIO.save("indices_d$D.jld2","indices",indices)
